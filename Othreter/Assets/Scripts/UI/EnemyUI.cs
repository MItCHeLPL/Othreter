...
public class EnemyUI : MonoBehaviour
{
	...
	void Update()
	{
		if (Vector3.Distance(transform.position, cam.transform.position) < hideDistance && ((enemyStats.currentHealth.GetValue() < enemyStats.maxHealth.GetValue() || enemyStats.currentArmor.GetValue() < enemyStats.maxArmor.GetValue()) || lockIndicator.enabled == true || ((Physics.Raycast(cam.transform.position, cam.transform.forward, out hit, hideDistance) && DataHolder.playerState_Aiming) ? (hit.transform == transform || hit.transform.parent == transform) : false)))
		{
